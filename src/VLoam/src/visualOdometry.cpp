                        ...
                        double &ox, double &oy, double &oz)
{
  ...
  double srycrx = sin(lx) * (cos(cy) * sin(cz) - cos(cz) * sin(cx) * sin(cy)) + cos(lx) * sin(ly) * (cos(cy) * cos(cz) + sin(cx) * sin(cy) * sin(cz)) + cos(lx) * cos(ly) * cos(cx) * sin(cy);
  double crycrx = cos(lx) * cos(ly) * cos(cx) * cos(cy) - cos(lx) * sin(ly) * (cos(cz) * sin(cy) - cos(cy) * sin(cx) * sin(cz)) - sin(lx) * (sin(cy) * sin(cz) + cos(cy) * cos(cz) * sin(cx));
  oy = atan2(srycrx / cos(ox), crycrx / cos(ox));

  double srzcrx = sin(cx) * (cos(lz) * sin(ly) - cos(ly) * sin(lx) * sin(lz)) + cos(cx) * sin(cz) * (cos(ly) * cos(lz) + sin(lx) * sin(ly) * sin(lz)) + cos(lx) * cos(cx) * cos(cz) * sin(lz);
  double crzcrx = cos(lx) * cos(lz) * cos(cx) * cos(cz) - cos(cx) * sin(cz) * (cos(ly) * sin(lz) - cos(lz) * sin(lx) * sin(ly)) - sin(cx) * (sin(ly) * sin(lz) + cos(ly) * cos(lz) * sin(lx));
...
void diffRotation(double cx, double cy, double cz, double lx, double ly, double lz,
                  double &ox, double &oy, double &oz)
{
  double srx = cos(cx) * cos(cy) * (sin(ly) * sin(lz) + cos(ly) * cos(lz) * sin(lx)) - cos(cx) * sin(cy) * (cos(ly) * sin(lz) - cos(lz) * sin(lx) * sin(ly)) - cos(lx) * cos(lz) * sin(cx);
  ox = -asin(srx);

  double srycrx = cos(cx) * sin(cy) * (cos(ly) * cos(lz) + sin(lx) * sin(ly) * sin(lz)) - cos(cx) * cos(cy) * (cos(lz) * sin(ly) - cos(ly) * sin(lx) * sin(lz)) - cos(lx) * sin(cx) * sin(lz);
  double crycrx = sin(cx) * sin(lx) + cos(cx) * cos(cy) * cos(lx) * cos(ly) + cos(cx) * cos(lx) * sin(cy) * sin(ly);
  oy = atan2(srycrx / cos(ox), crycrx / cos(ox));

  double srzcrx = cos(cx) * cos(lx) * cos(lz) * sin(cz) - (cos(cz) * sin(cy) - cos(cy) * sin(cx) * sin(cz)) * (sin(ly) * sin(lz) + cos(ly) * cos(lz) * sin(lx)) - (cos(cy) * cos(cz) + sin(cx) * sin(cy) * sin(cz)) * (cos(ly) * sin(lz) - cos(lz) * sin(lx) * sin(ly));
  double crzcrx = (sin(cy) * sin(cz) + cos(cy) * cos(cz) * sin(cx)) * (sin(ly) * sin(lz) + cos(ly) * cos(lz) * sin(lx)) + (cos(cy) * sin(cz) - cos(cz) * sin(cx) * sin(cy)) * (cos(ly) * sin(lz) - cos(lz) * sin(lx) * sin(ly)) + cos(cx) * cos(cz) * cos(lx) * cos(lz);
  oz = atan2(srzcrx / cos(ox), crzcrx / cos(ox));
}
...
void imagePointsHandler(const sensor_msgs::PointCloud2ConstPtr &imagePoints2)
{
  ...
  if (imuPointerLast >= 0)
  {
    ...
    else
    {
      ...
      double ratioFront = (imagePointsCurTime - imuTime[imuPointerBack]) / (imuTime[imuPointerFront] - imuTime[imuPointerBack]);
      double ratioBack = (imuTime[imuPointerFront] - imagePointsCurTime) / (imuTime[imuPointerFront] - imuTime[imuPointerBack]);
    ...
    if (ipFound)
    {
      ...
      if (depthCloudNum > 10)
      {
        ...
        if (pointSearchSqrDis[0] < 0.5 && pointSearchInd.size() == 3)
        {
          ...
          // 其他没有找到匹配的激光深度点都将ipr.s和ipr.v设为0
        }
        else
        {
          ipr.s = 0;
      ...
      if (fabs(ipr.v) < 0.5)
      {
        ...
        if (sqrt(disX * disX + disY * disY + disZ * disZ) > 1)
        {
          ...
          double delta = sqrt((v0 - v1) * (v0 - v1) + (u0 - u1) * (u0 - u1)) * cos(atan2(tz * v1 - ty, tz * u1 - tx) - atan2(v0 - v1, u0 - u1));
  ...
  for (int iterCount = 0; iterCount < iterNum; iterCount++)
  {
    ...
    for (int i = 0; i < ipRelationsNum; i++)
    {
      ...
      if (fabs(ipr.v) < 0.5)
      {
        ...
        // 对roll的偏导数
        ipr2.x = v0 * (crz * srx * (tx - tz * u1) - crx * (ty * u1 - tx * v1) + srz * srx * (ty - tz * v1)) - u0 * (sry * srx * (ty * u1 - tx * v1) + crz * sry * crx * (tx - tz * u1) + sry * srz * crx * (ty - tz * v1)) + cry * srx * (ty * u1 - tx * v1) + cry * crz * crx * (tx - tz * u1) + cry * srz * crx * (ty - tz * v1);
        // 对pitch的偏导数
        ipr2.y = u0 * ((tx - tz * u1) * (srz * sry - crz * srx * cry) - (ty - tz * v1) * (crz * sry + srx * srz * cry) + crx * cry * (ty * u1 - tx * v1)) - (tx - tz * u1) * (srz * cry + crz * srx * sry) + (ty - tz * v1) * (crz * cry - srx * srz * sry) + crx * sry * (ty * u1 - tx * v1);
        // 对yaw的偏导数
        ipr2.z = -u0 * ((tx - tz * u1) * (cry * crz - srx * sry * srz) + (ty - tz * v1) * (cry * srz + srx * sry * crz)) - (tx - tz * u1) * (sry * crz + cry * srx * srz) - (ty - tz * v1) * (sry * srz - cry * srx * crz) - v0 * (crx * crz * (ty - tz * v1) - crx * srz * (tx - tz * u1));
        // 对tx的偏导数
        ipr2.h = cry * crz * srx - v0 * (crx * crz - srx * v1) - u0 * (cry * srz + crz * srx * sry + crx * sry * v1) - sry * srz + crx * cry * v1;
        // 对ty的偏导数
        ipr2.s = crz * sry - v0 * (crx * srz + srx * u1) + u0 * (cry * crz + crx * sry * u1 - srx * sry * srz) - crx * cry * u1 + cry * srx * srz;
        // 对tz的偏导数
        ipr2.v = u1 * (sry * srz - cry * crz * srx) - v1 * (crz * sry + cry * srx * srz) + u0 * (u1 * (cry * srz + crz * srx * sry) - v1 * (cry * crz - srx * sry * srz)) + v0 * (crx * crz * u1 + crx * srz * v1);
        ...
        double y2 = (ty - tz * v1) * (crz * sry + cry * srx * srz) - (tx - tz * u1) * (sry * srz - cry * crz * srx) - v0 * (srx * (ty * u1 - tx * v1) + crx * crz * (tx - tz * u1) + crx * srz * (ty - tz * v1)) + u0 * ((ty - tz * v1) * (cry * crz - srx * sry * srz) - (tx - tz * u1) * (cry * srz + crz * srx * sry) + crx * sry * (ty * u1 - tx * v1)) - crx * cry * (ty * u1 - tx * v1);
        ...
        if (ptNumNoDepthRec < 50 || iterCount < 25 || fabs(y2) < 2 * meanValueWithDepthRec / 10000)
        {
          ...
          ipr2.v *= scale;
          y2 *= scale;
          ipRelations2->push_back(ipr2); // ipr2分量[r1,r2,r3,t1,t2,t3]的偏导数
          ipy2.push_back(y2);            // R_{(i-1)toi}的逆
      ...
      else if (fabs(ipr.v - 1) < 0.5 || fabs(ipr.v - 2) < 0.5)
      {
        ...
        // 以下为y3的偏导数
        ipr3.x = d0 * (cry * srz * crx + cry * u1 * srx) - d0 * u0 * (sry * srz * crx + sry * u1 * srx) - d0 * v0 * (u1 * crx - srz * srx);
        ...
        double y3 = tx - tz * u1 + d0 * (crz * sry - crx * cry * u1 + cry * srx * srz) - d0 * v0 * (crx * srz + srx * u1) + d0 * u0 * (cry * crz + crx * sry * u1 - srx * sry * srz);

        // 以下为y4的偏导数
        ipr4.x = d0 * (cry * v1 * srx - cry * crz * crx) + d0 * u0 * (crz * sry * crx - sry * v1 * srx) - d0 * v0 * (crz * srx + v1 * crx);
        ...
        double y4 = ty - tz * v1 - d0 * (cry * crz * srx - sry * srz + crx * cry * v1) + d0 * v0 * (crx * crz - srx * v1) + d0 * u0 * (cry * srz + crz * srx * sry + crx * sry * v1);
    ...
    int ipRelations2Num = ipRelations2->points.size(); // ipRelations2 已经匹配的计算出约束的特征点
    ...
    if (ipRelations2Num > 10)
    {
      ...
      for (int i = 0; i < ipRelations2Num; i++)
      {
        ...
        matB.at<float>(i, 0) = -0.2 * ipy2[i]; // -0.2为优化步长,由于是下降，因此取负号
      }
      ...
      float deltaR = sqrt(matX.at<float>(0, 0) * 180 / PI * matX.at<float>(0, 0) * 180 / PI + matX.at<float>(1, 0) * 180 / PI * matX.at<float>(1, 0) * 180 / PI + matX.at<float>(2, 0) * 180 / PI * matX.at<float>(2, 0) * 180 / PI);
      float deltaT = sqrt(matX.at<float>(3, 0) * 100 * matX.at<float>(3, 0) * 100 + matX.at<float>(4, 0) * 100 * matX.at<float>(4, 0) * 100 + matX.at<float>(5, 0) * 100 * matX.at<float>(5, 0) * 100);
  ...
  depthPointsCurNum = 2; // depthPointsCur前两个点存放了旋转和偏移，因此depthPointsCurNum初始为2
...
void depthCloudHandler(const sensor_msgs::PointCloud2ConstPtr &depthCloud2)
{
  ...
  if (depthCloudNum > 10)
  {
    for (int i = 0; i < depthCloudNum; i++)
    {
      ...
      // intensity存放点云的原始深度信息，xyz存放深度为化为10的坐标
...
void imageDataHandler(const sensor_msgs::Image::ConstPtr &imageData)
{
  ...
  for (int i = 0; i < ipRelationsNum; i++)
  {
    if (fabs(ipRelations->points[i].v) < 0.5)
    {
      ...
      cv::circle(bridge->image, cv::Point((kImage[2] - ipRelations->points[i].z * kImage[0]) / showDSRate, (kImage[5] - ipRelations->points[i].h * kImage[4]) / showDSRate), 1, CV_RGB(255, 0, 0), 2);
    }
    else if (fabs(ipRelations->points[i].v - 1) < 0.5)
    {
      // 通过点云获得深度的图像点，显示绿色
      cv::circle(bridge->image, cv::Point((kImage[2] - ipRelations->points[i].z * kImage[0]) / showDSRate, (kImage[5] - ipRelations->points[i].h * kImage[4]) / showDSRate), 1, CV_RGB(0, 255, 0), 2);
    }
    else if (fabs(ipRelations->points[i].v - 2) < 0.5)
    {
      // 通过相机运动获得深度的图像点，显示蓝色
      cv::circle(bridge->image, cv::Point((kImage[2] - ipRelations->points[i].z * kImage[0]) / showDSRate, (kImage[5] - ipRelations->points[i].h * kImage[4]) / showDSRate), 1, CV_RGB(0, 0, 255), 2);
    } /*else {
