...
void voDataHandler(const nav_msgs::Odometry::ConstPtr &voData)
{
  ...
  if (time - timeRec < 0.5)
  {
    ...
    downSizeFilter.setLeafSize(0.05, 0.05, 0.05); // 点云占据体素最小尺寸为边长0.05m的立方体，1个边长0.05m的立方体只保存一个点
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber syncCloudSub = nh.subscribe<sensor_msgs::PointCloud2>("/sync_scan_cloud_filtered", 5, syncCloudHandler);
